...
$transcriptPath = Join-Path "logs" "transcript_phase2_$ts.txt"
...
# Phase 2: unfreeze last stage (layer4) + head
$device = "cpu"
$epochs = 10
$batch = 16
$lr = 3e-4
...
Write-Host "`n=== TRAIN (PHASE 2: unfreeze-last=1, lr=0.0003) ==="
$trainLogPath = Join-Path "logs" "train_phase2_$ts.txt"
...
Write-Host "`n=== PREDICT (val/cat folder: first image) ==="
python predict.py --image "data\val\cat"  --checkpoint $bestCkpt --device $device --topk 2

Write-Host "`n=== PREDICT (val/fish folder: first image) ==="
python predict.py --image "data\val\fish" --checkpoint $bestCkpt --device $device --topk 2

Write-Host "`n=== PREDICT (random sample from val) ==="
